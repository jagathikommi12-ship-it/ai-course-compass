...
# individually (e.g. a spring-start transfer student just removes the initial
# "Year 1 - Fall" and adds a Spring term instead).
DEFAULT_TARGET_SEMESTERS = 8


def _ensure_plan_exists(user_id: str) -> dict:
    """
    Lazily creates plan settings + the default 8-term sequence the first
    time a user's plan is touched, so the calendar is populated immediately
    instead of requiring an explicit "set up your plan" step.
    """
    settings_row = catalog_repo.fetch_plan_settings(user_id)
    if settings_row is None:
        settings_row = catalog_repo.upsert_plan_settings(user_id, incoming_credits=0)
    if not catalog_repo.fetch_terms(user_id):
        catalog_repo.insert_terms(user_id, generate_regular_terms(DEFAULT_TARGET_SEMESTERS))
    return settings_row
...
def get_plan(program_id: str | None = None, user: CurrentUser = Depends(get_current_user)):
    settings_row = _ensure_plan_exists(user.user_id)
    ...
    summary = compute_credits_summary(total_required, settings_row["incoming_credits"], planned, courses_by_code)

    return PlanOut(
        settings=PlanSettingsOut(**settings_row),
...
def update_plan_settings(body: PlanSettingsIn, user: CurrentUser = Depends(get_current_user)):
    """Updates the incoming (AP/IB/transfer) credits count."""
    row = catalog_repo.upsert_plan_settings(user.user_id, body.incoming_credits)
    return PlanSettingsOut(**row)
